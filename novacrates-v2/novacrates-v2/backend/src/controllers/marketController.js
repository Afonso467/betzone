...
const buySkin = asyncHandler(async (req, res) => {
  ...
  await UserModel.adjustPoints(DEMO_USER_ID, -skin.points_value);
  await InventoryModel.add(DEMO_USER_ID, skin.id);
...
const sellSkin = asyncHandler(async (req, res) => {
  ...
  const inventory = await InventoryModel.getByUser(DEMO_USER_ID);
  ...
  await InventoryModel.remove(inventoryId, DEMO_USER_ID);
  await UserModel.adjustPoints(DEMO_USER_ID, sellPoints);
  const updated = await UserModel.findById(DEMO_USER_ID);
...
const getGiveaways = asyncHandler(async (req, res) => {
  ...
  for (const g of giveaways) {
    g.entered = await GiveawayModel.hasEntered(g.id, DEMO_USER_ID);
  }
  res.json({ giveaways });
});
...
const enterGiveaway = asyncHandler(async (req, res) => {
  ...
  const already = await GiveawayModel.hasEntered(id, DEMO_USER_ID);
  if (already) throw createError('Já participas neste giveaway', 400);
  await GiveawayModel.enter(id, DEMO_USER_ID);
  await UserModel.addXP(DEMO_USER_ID, XP_REWARDS.giveaway_enter);
...
const getLeaderboard = asyncHandler(async (req, res) => {
  ...
  const rank = await LeaderboardModel.getUserRank(DEMO_USER_ID);
